...
def get_task_name(args):
    if args.algo == 'bc':
        ...
        task_name = task_name + ".g_step_" + str(args.g_step) + ".d_step_" + str(args.d_step) + \
                    ".policy_entcoeff_" + str(args.policy_entcoeff) + ".adversary_entcoeff_" + str(
...
def main(args):
    ...
    def policy_fn(name, ob_space, ac_space, reuse=False):
        return mlp_policy.MlpPolicy(name=name, ob_space=ob_space, ac_space=ac_space,
                                    reuse=reuse, hid_size=64, num_hid_layers=3)
    ...
    # discriminator
    discriminator = TransitionClassifier((18,), args.adversary_hidden_size, entcoeff=args.adversary_entcoeff)
    if args.algo == 'trpo':
        # Set up for MPI seed
